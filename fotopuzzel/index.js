...
REDIPS.drag=(function()
{
    ...
    var scroll_object,bgstyle_old,
        scrollable_container=[],tables=[],sort_idx,
        moved,cloned,cloned_id=[],currentCell=[],
        div_drag=null,div_box=null;
    var pointer=
    {
        x:0,y:0
    },
    threshold=
    {
        x:0,y:0,value:7,flag:false
    },
    shift_key=false,clone_class=false,table=null,table_old=null,
    table_source=null,row=null,row_old=null,row_source=null,
    cell=null,cell_old=null,
    cell_source=null,obj=false,obj_old=false,mode='cell',
    hover=
    {
    ...
    mark=
    {
        action:'deny',cname:'mark',exception:[]
    },
    border='solid',border_disabled='dotted',opacity_disabled,
    trash_cname='trash',trash_ask=true,trash_ask_row=true,
    drop_option='single',shift_option='horizontal1',
    multiple_drop='bottom',delete_cloned=true,
    delete_shifted=false,source_cell=null,current_cell=null,
    previous_cell=null,target_cell=null,animation_pause=20,
    animation_step=2,animation_shift=false,shift_after=true,
    an_counter=0,clone_shiftKey=false,clone_shiftKey_row=false,
    row_empty_color='White';
    init=function(dc)
    {
        var self=this,
        i,imgs,redips_clone;
...
{
    var table_mini,offset,row_obj,last_idx,empty_row=true,cr,div,i,j;
    if(el.nodeName==='DIV')
    {
        div=el;
...
src=table_mini.redips.source_row;rowIndex=src.rowIndex;
...
if(!animated){REDIPS.drag.myhandler_row_dropped(target_cell);}}
...
{
var tr,i,s;if(mode==='cell'&&threshold.flag)
{
...
edge.flag.y=0;currentCell=[0,0,0,0];
}
};
clone_div=function(div,drag)
{
    var div_cloned=div.cloneNode(true),offset,offset_dragged;
    if(drag===true)
    {
...
cloned_id[div.id]+=1;copy_properties(div,div_cloned);
return(div_cloned);};copy_properties=function(src,cln)
{
    var copy=[],childs;
...
if(limit<=0){classes=classes.replace('clone','');
if(limit_type===2){classes=classes.replace('drag','');
obj_old.onmousedown=null;
obj_old.ontouchstart=null;obj_old.style.cursor='auto';
...
return flag;};enable_drag=function(enable_flag,el,type)
{
    ...
    position,regex_drag=/\bdrag\b/i,
    regex_noautoscroll=/\bnoautoscroll\b/i;
...
divs[i].onmousedown=handler1;divs[i].ontouchstart=handler1;
divs[i].ondblclick=handler2;
divs[i].style.borderStyle=borderStyle;
divs[i].style.cursor=cursor;divs[i].redips.enabled=enabled;}
else if(enable_flag==='init'){overflow=get_style(divs[i],'overflow');
if(overflow!=='visible')
{
...
scrollable_container[j]={div:divs[i],offset:cb,
...
return false;};row_opacity=function(el,opacity,color)
{
    var td,i,j;
    if(typeof(el)==='string')
    {
        el=document.getElementById(el);el=find_parent('TABLE',el);
    }
if(el.nodeName==='TR')
{
...
{
var tbl=document.getElementById(tbl_id),row=tbl.rows[row_idx];
if(color===undefined)
...
row.redips.empty_row=true;row_opacity(row,'empty',color);
};
return{
    obj:obj,
    obj_old:obj_old,
